...
import { FOLLOW_USER, GET_FOLLOWERS_OF_USERID, GET_FOLLOWING_OF_USERID } from '../../constants/ApiEndpoints';



const FollowAndFollowingModal = ({isFollow, userId, setIsFollows, setShowFollowAndFollowingModal}) => {
    ...
    return ( 
        <div onClick={()=>setShowFollowAndFollowingModal(false)} className='single-modal'>
            <div onClick={handleModalClick} className='single-modal-content'>
                <div className='single-modal-header'>
                    ...
                    <div className="following-tag">
                        ...
                        <span onClick={handleFollowingClick} className="header-tag" id="following-tag">following</span>
                    </div>
                </div>
                <div className="modal-content">
                    { isFollow && followers.map(follower => (
                        <div key={follower.User.FMNO} className="modal-list">
                            <div className="img-name-desg-container">
                                ...
                                <div className="name-desg-container">
                                    ...
                                    <span className="designation">{follower.User.designation}</span>
                                </div>
                            </div>
                            <div className="follow-btn-container">
                                {console.log(follower.User.isFollowed==='0', follower.User.isFollowed)}
                                {follower.User.isFollowed==='0' ? <button onClick={()=>handleFollowBtnClick(follower.User.FMNO)} className="action-btn" id="follow-btn">follow</button> : <button className="action-btn">Unfollow</button>}
                                {/* <button className="follow-btn">follow</button> */}
                            </div>
                        </div>
                    ))}
                    { !isFollow && following.map(follower => (
                        <div key={follower.User.FMNO} className="modal-list">
                            <div className="img-name-desg-container">
                                <img id="profile-img" src={profileImage} alt="" />
                                <div className="name-desg-container">
